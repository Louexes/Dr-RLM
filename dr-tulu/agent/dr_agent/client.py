...
import litellm
# GPT-5 / reasoning models reject `stop` and `top_p`; litellm's own error message
# recommends drop_params to silently strip unsupported params per-model. Parser-mode
# tool-calling still works without `stop` (the parser scans the full text for the
# first tool call); the model just over-generates slightly. Reversible via git.
