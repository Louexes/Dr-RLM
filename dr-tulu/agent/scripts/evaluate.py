...
def run_evaluation(
    ...
    # Setup grader. Gemini graders are thinking models: reasoning tokens eat
    # into max_tokens, so a 1000 cap truncates the JSON value mid-string on long
    # (multi-turn) conversations -> infinite "bad JSON" retry. Disable thinking
    # and give generous headroom for the explanation.
    _is_gemini = "gemini" in str(grader_model).lower()
    grader_sampler = ChatCompletionSampler(
        model=grader_model,
        system_message=OPENAI_SYSTEM_MESSAGE_API,
        max_tokens=4096 if _is_gemini else 1000,
        temperature=0,
        reasoning_effort="none" if _is_gemini else None,
    )
    ...
    elif task in ["2wiki", "webwalker", "dsqa", "webshaper"]:
