...
def parse_answer(sample: str) -> List[Dict[str, Any]]:
    ...
    # remove empty sections and sections that are just a single heading line
    # (sec.strip()[0] on an empty section -> "string index out of range")
    raw_sections = [
        sec for sec in raw_sections
        if sec.strip() and not (sec.strip()[0] == '#' and len(sec.strip().split('\n')) == 1)
    ]

    # get all snippets from the thinking section (robust to schemas where per-tool-call
    # entries lack 'generated_text', e.g. native tool-calling agents — A3/gpt-5-mini)
    ft = sample.get('full_traces', {}) or {}
    gen_text = ft.get('generated_text', '') or ''
    if '<think>' in gen_text:
        snippets = extract_citations_from_context(gen_text)
    else:
        concat_text = ""
        for tool_call in (ft.get('tool_calls', []) or []):
            concat_text += (tool_call.get('generated_text', '') if isinstance(tool_call, dict) else '') + "\n"
