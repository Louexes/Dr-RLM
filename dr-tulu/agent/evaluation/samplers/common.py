...
def map_with_progress(
    f: Callable,
    xs: list[Any],
    num_threads: int = min(os.cpu_count() or 10, 8),
    pbar: bool = True,
):
    """
    Apply f to each element of xs, using a ThreadPool, and show progress.
    """
    # ponytail: cap at 8. HealthBench nests this pool (examples × each example's
    # rubrics); uncapped on a 96-core node → ~thousands of threads → "can't start
    # new thread". 8×8=64 peak is thread- and Gemini-rate-safe. patches_to_deps.md.
