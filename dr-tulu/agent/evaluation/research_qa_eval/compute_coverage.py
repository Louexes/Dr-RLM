...
def compute_coverage(
    ...
    for item in tqdm(items, desc="Computing rubric coverage"):
        ...
        # An empty/whitespace report addresses NOTHING -> coverage 0. Short-circuit BEFORE the judge:
        # sending an empty "Response:" makes the LLM judge hallucinate "Completely" on every rubric
        # (observed: empty report -> coverage 1.000), which would inflate any baseline that contains
        # empty reports (the untrained model produces ~50% empties). See docs/patches_to_deps.md 2026-06-28.
        if not (answer or "").strip():
            rubric_judges = [{"rubric": r, "score": "Not at all"} for r in rubric_items]
            results[item_id] = rubric_judges
            item_coverage = compute_coverage_score(rubric_judges)
            coverages.append(item_coverage)
            logger.info(f"Coverage for {item_id}: {item_coverage:.3f} (empty report -> 0, judge skipped)")
            continue
