...
from dr_agent.tool_interface.local_bm25s import (
    LocalBm25sBrowseTool,
    LocalBm25sSearchTool,
)
...
class SearchAgent(BaseAgent):
    ...
    def prompt(
        ...
        # DR-RLM STEELMAN PATCH (see dr-rlm/docs/patches_to_deps.md): per-example
        # additional_instructions from the dataset loaders were silently dropped
        # (workflow.generate_dataset filters fields to __call__'s signature, and neither
        # __call__ nor prompt() accepted them) — the agent only ever saw the generic YAML
        # category instruction (e.g. short_form "write a short paragraph" for DRB). For
        # task-statement parity with DR-RLM, an explicit per-example instruction now takes
        # precedence over the YAML category text.

        PROMPT = UNIFIED_TOOL_CALLING_STRUCTURED_PROMPTS[self.prompt_version]
        system_prompt = PROMPT["system_prompt"]

        # DR-RLM STEELMAN PATCH (part 2): DR_TULU_FRAMING_JSON env var maps dataset_name ->
        # task statement, with HIGHEST precedence. Exporting the same JSON used for DR-RLM's
        # _TASK_FRAMING gives both systems byte-identical task statements in head-to-heads.
        _env_map = os.environ.get("DR_TULU_FRAMING_JSON")
        if _env_map:
            try:
                _text = json.loads(_env_map).get(dataset_name or "")
                if _text:
...
class AutoReasonSearchWorkflow(BaseWorkflow):
    ...
    class Configuration(BaseWorkflowConfiguration):
        ...
        # "parser" (stop-token tag parsing, default; how DR-Tulu-8B is trained) or
        # "native" (OpenAI-style function-calling; required for GPT-5/reasoning models
        # that reject `stop` and unreliably emit the tag format). Default keeps A1 intact.
        ...
        # Local bm25s frozen-corpus retrieval (search_tool_name/browse_tool_name == "bm25s").
        # Reuses the RLM's in-process _Bm25sBackend for byte-identical retrieval (controlled
        # comparison vs DR-RLM). No MCP server needed. See dr_agent/tool_interface/local_bm25s.py.
        bm25s_index_path: Optional[str] = None
    ...
    def before_launch_check(self) -> None:
        ...
        # Local bm25s retrieval runs in-process (no MCP server). Skip the MCP check
        # entirely so a batch job doesn't block on the interactive Confirm.ask.
        uses_mcp = not (
            getattr(cfg, "search_tool_name", "serper") == "bm25s"
            and getattr(cfg, "browse_tool_name", None) in ("bm25s", None)
        )

        # Check MCP server
        mcp_port = getattr(cfg, "mcp_port", 8000)
        if uses_mcp and not check_port(mcp_port):
            console.print(
            ...
            console.print(
                "[green]✓[/green]  Local bm25s retrieval (in-process) — no MCP server needed"
            )
        else:
            console.print(
    ...
    def setup_components(
        ...
        elif cfg.search_tool_name == "bm25s":
            # In-process frozen-corpus retrieval (controlled comparison vs DR-RLM).
            # Both tool names point at the SAME bm25s backend the model was prompted with.
            assert cfg.bm25s_index_path, "bm25s_index_path is required for search_tool_name=bm25s"
            self.search_tool = LocalBm25sSearchTool(
                tool_parser=cfg.tool_parser,
                number_documents_to_search=cfg.number_documents_to_search,
                timeout=cfg.search_timeout,
                name="snippet_search",
                search_index_path=cfg.bm25s_index_path,
                search_corpus_path=cfg.bm25s_corpus_path,
            )
            self.search_tool2 = LocalBm25sSearchTool(
                tool_parser=cfg.tool_parser,
                number_documents_to_search=cfg.number_documents_to_search,
                timeout=cfg.search_timeout,
                name="google_search",
        ...
        if cfg.browse_tool_name == "bm25s":
            self.browse_tool = LocalBm25sBrowseTool(
                tool_parser=cfg.tool_parser,
        ...
        elif cfg.browse_tool_name == "serper":
    ...
    async def __call__(
        ...
        # DR-RLM STEELMAN PATCH: accepting this here lets workflow.generate_dataset's
        # signature-based field filter pass the loaders' per-example instructions through
        # (they were built by every loader but silently dropped before reaching the agent).
        additional_instructions: Optional[str] = None,
    ) -> Dict[str, Any]:
        cfg = self.configuration
        assert cfg is not None
        ...
        results = await self.search_agent(
            ...
            history=history,
            additional_instructions=additional_instructions,  # DR-RLM steelman patch
            ...
            tool_calling_mode=getattr(cfg, "tool_calling_mode", "parser"),
            verbose=verbose,
