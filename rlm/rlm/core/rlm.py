...
class RLM:
    ...
    def __init__(
        ...
        # DR-RLM PATCH (see dr-rlm/docs/patches_to_deps.md): optional distinct system prompt
        # for spawned children (depth-banded orchestrator/worker parity with DrRlmEnv).
        # None -> children inherit self.system_prompt (original behavior, unchanged).
    ...
    def _subcall(self, prompt: str, model: str | None = None) -> RLMChatCompletion:
        ...
        child = RLM(
            ...
            custom_system_prompt=self.sub_system_prompt or self.system_prompt,
            sub_system_prompt=self.sub_system_prompt,  # grandchildren keep the child band
            orchestrator=self.orchestrator,  # propagate so children don't re-append a baked-in addendum
