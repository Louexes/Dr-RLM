...
def prepare_runtime_environment(cfg: SkyRLTrainConfig) -> dict[str, str]:
    ...
    # DR-RLM: forward the dr_rlm knobs + judge key into Ray workers (set-only — a no-op
    # for runs that don't set them). prompts.py (DR_RLM_PROMPT_FILE), the credit telemetry
    # hook (DR_RLM_CREDIT_METRICS_PATH), rer_advantage (DR_RLM_DEPTH_*), and judge.py
    # (judge_api_key_env, e.g. JUDGE_API_KEY) all read os.environ INSIDE workers, where the
    # submitting shell's environment is not inherited.
    for var_name in list(os.environ):
        if var_name.startswith("DR_RLM_"):
            logger.info(f"Exporting `{var_name}` to ray runtime env")
            env_vars[var_name] = os.environ[var_name]
    if os.environ.get("JUDGE_API_KEY"):
        logger.info("Exporting JUDGE_API_KEY to ray runtime env")
        env_vars["JUDGE_API_KEY"] = os.environ["JUDGE_API_KEY"]
    if os.environ.get("PYTORCH_CUDA_ALLOC_CONF"):
        logger.info("Exporting PYTORCH_CUDA_ALLOC_CONF to ray runtime env")
        env_vars["PYTORCH_CUDA_ALLOC_CONF"] = os.environ["PYTORCH_CUDA_ALLOC_CONF"]
    if os.environ.get("VLLM_USE_DEEP_GEMM") is not None:
        logger.info(f"Exporting VLLM_USE_DEEP_GEMM={os.environ['VLLM_USE_DEEP_GEMM']} to ray runtime env")
        env_vars["VLLM_USE_DEEP_GEMM"] = os.environ["VLLM_USE_DEEP_GEMM"]
    if os.environ.get("HF_HUB_OFFLINE"):
        logger.info("Exporting HF_HUB_OFFLINE to ray runtime env")
        env_vars["HF_HUB_OFFLINE"] = os.environ["HF_HUB_OFFLINE"]
