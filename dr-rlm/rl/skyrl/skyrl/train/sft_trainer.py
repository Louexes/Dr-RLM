...
class SFTTrainer:
    ...
    def _load_and_tokenize(self, dataset_name: str, dataset_split: str) -> list:
        ...
        # Local files (.jsonl/.json/.parquet) must go through the builder + data_files form;
        # passing the bare path as the first arg raises FileNotFoundError. Mirror the RL
        # PromptDataset loader so SFT can consume our locally-generated trajectory jsonl.
        _ext = os.path.splitext(dataset_name)[-1].lower()
        if _ext in (".jsonl", ".json"):
    ...
    def train(self):
        ...
        while self.global_step <= num_steps:
            ...
            log_dict = {
                "train/loss": step_result["loss"],
                "train/perplexity": exp(min(step_result["loss"], 20.0)),
                "train/epoch": self.global_step / max(1, steps_per_epoch),
                "train/grad_norm": step_result["grad_norm"],
                "train/tokens_per_second": tokens_per_second,
                "train/actual_num_tokens": actual_num_tokens,
