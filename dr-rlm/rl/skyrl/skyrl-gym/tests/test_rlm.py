...
# ---------------------------------------------------------------------------
# 4. PersistentREPL — answer-dict final-answer capture (upstream RLM contract)
# ---------------------------------------------------------------------------


class TestAnswerDict:
    def test_answer_ready_captures_final_answer(self):
        repl = PersistentREPL()
        result = repl.execute('answer["content"] = "Paris"\nanswer["ready"] = True')
        assert result.final_answer == "Paris"
        assert result.final_obj == {"content": "Paris", "citations": []}

    def test_no_final_answer_until_ready(self):
        """Setting content without flipping ready must NOT terminate."""
        repl = PersistentREPL()
        result = repl.execute('answer["content"] = "Paris"')
        assert result.final_answer is None
        assert result.final_obj is None

    def test_answer_content_built_across_turns(self):
        """content can be accumulated over multiple cells before ready flips."""
        repl = PersistentREPL()
        repl.execute('answer["content"] = "draft"')
        result = repl.execute('answer["content"] += " final"\nanswer["ready"] = True')
        assert result.final_answer == "draft final"

    def test_list_content_round_trips_as_str(self):
        """The evidence env submits a list; it must stringify exactly like FINAL_VAR(list) did,
        so the reward / child-return ast.literal_eval round-trip is preserved."""
        repl = PersistentREPL()
        result = repl.execute('answer["content"] = ["a", "b"]\nanswer["ready"] = True')
        assert result.final_answer == "['a', 'b']"

    def test_citations_captured_when_supplied(self):
        """Optional citations (DR-RLM ledger_support ablation) are snapshotted on ready."""
        repl = PersistentREPL()
        result = repl.execute('answer["content"] = "r"\nanswer["citations"] = ["s1"]\nanswer["ready"] = True')
        assert result.final_obj == {"content": "r", "citations": ["s1"]}

    def test_rebind_to_plain_dict_still_captured(self):
        """If the model rebinds answer to a plain dict with ready=True, the restore-scaffold
        fallback still captures it (and re-wraps it for the next cell)."""
        repl = PersistentREPL()
        result = repl.execute('answer = {"content": "X", "ready": True}')
        assert result.final_answer == "X"

    def test_answer_is_reserved_against_custom_tools(self):
        """`answer` is reserved, so a custom tool cannot shadow it."""
        try:
            PersistentREPL(custom_tools={"answer": lambda: None})
            assert False, "expected ValueError for reserved name 'answer'"
        except ValueError:
            pass


class TestEnvAnswerSubmission:
    def _make_env(self):
        extras = {
            "reward_spec": {"ground_truth": "Paris"},
            "max_turns": 5,
    ...
    def test_step_with_answer_dict_terminates(self):
        env = self._make_env()
        env.init([{"role": "user", "content": "What is the capital of France?"}])
        action = '```repl\nanswer["content"] = "Paris"\nanswer["ready"] = True\n```'
        step_out = env.step(action)
        assert step_out["done"] is True
        assert step_out["reward"] == 1.0  # base _get_reward: 1.0 for any non-empty answer
        metrics = env.get_metrics()
        assert metrics["final_answer"] == "Paris"
        assert metrics["final_value_set"] is True

    def test_step_without_answer_continues(self):
        env = self._make_env()
        env.init([{"role": "user", "content": "What is the capital of France?"}])
        action = "```repl\nprint(context)\n```"
        step_out = env.step(action)
...
class TestREPLMemoryGuard:
    """A runaway model-written cell must raise MemoryError inside the cell instead of
    inflating the host process until Ray OOM-kills the trainer (job 24457033, 535 GB)."""

    def test_runaway_allocation_returns_memoryerror_and_repl_survives(self):
        import os
        if os.environ.get("RLM_REPL_MEM_CAP_GB") is None:
            os.environ["RLM_REPL_MEM_CAP_GB"] = "8"
        import skyrl_gym.envs.rlm.repl as repl_mod
        repl_mod._MEM_GUARD_INSTALLED = False  # re-install with the test cap
        r = repl_mod.PersistentREPL(timeout=30)
        res = r.execute("x = bytearray(64 * 1024**3)")
        assert "MemoryError" in res.stderr
        assert r.execute("print(1 + 1)").stdout.strip() == "2"
