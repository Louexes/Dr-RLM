    ...
    * ``_post_process_agent_loop_output`` — stamp per-step RLM metadata, stash
    ...
    def _make_subcall_fn(
        ...
        async def _run_child(prompt: str, context=None) -> str:
            ...
            return self._child_return_value(child_env_metrics)

        def subcall_fn(prompt: str, context=None):
    ...
    def _child_return_value(self, child_env_metrics: Dict[str, Any]) -> Any:
        """What a sub-agent's ``rlm_query()`` returns to the parent's REPL. Base: the
        child's final-answer string. ``DrRlmGenerator`` overrides this to return the
        structured ``answer`` dict so the parent can merge the child's citations
        programmatically instead of copying ``<cite>`` substrings out of prose."""
