...
- `answer` dict — submit your final answer by setting `answer["content"]` to your list of evidence strings, then `answer["ready"] = True`.
...
**Final turn**: Flatten all child results into a single list of evidence strings and submit it. Do NOT filter or verify.
```repl
evidence = []
for r in results1:
...
- NEVER submit (set answer["ready"] = True) in the same block as extract_section. You must first extract, READ the output \
to verify it looks correct, and ONLY THEN submit in the next block.
- Your final answer MUST be a list of strings — each an exact slice of `context` — placed in answer["content"].
...
Turn 5 — verify the extractions look correct, then submit:
```repl
answer["content"] = [p1, p2]
answer["ready"] = True
```\
"""
)
