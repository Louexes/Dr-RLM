...
# =============================================================================
# DR-RLM ablation ladder — RUNG L2 : RAO LOCAL-NODE + DEPTH WEIGHTING
# -----------------------------------------------------------------------------
# Proposal §6 ladder (Recursive Deep Research Proposal.md, "Ablation ladder"):
#   | L2 | + RAO delegation bonus + depth weighting | coarse scalar per-node |
#
# L2 isolates: the RAO scheme — a COARSE per-node scalar (own-answer quality +
# lambda * mean-child-success, the delegation bonus) plus the RAO depth
# inverse-frequency weighting in the advantage estimator. It is per-node credit,
# but NOT provenance-attributed: it has no idea which child's evidence actually
# reached the report. RQ2 = L3 (provenance) vs {L1 (inherited), L2 (RAO)}.
#
# Mechanism knobs that make this rung L2:
#   generator.per_node_credit=true       -> un-flatten: each node is its own trajectory
#                                           (this also forces train_child_trajectories=true
#                                           in DrRlmGeneratorConfig.__post_init__)
#   generator.reward_mode=rao            -> r_a = s~(own) + rao_lambda * mean(child s~) - gamma*cost
#   trainer.algorithm.advantage_estimator=rer_pernode  -> RAO depth inverse-frequency weighting
#   export DR_RLM_DEPTH_WEIGHTING=1      -> enable the w_d = alpha/N_d weighting (rer_advantage.py)
#   export DR_RLM_DEPTH_WEIGHT_ALPHA=1.0 -> the alpha in w_d (read by the estimator)
#
...
# ----- L2-specific: RAO depth inverse-frequency weighting (read by rer_advantage.py) -----
export DR_RLM_DEPTH_WEIGHTING="${DR_RLM_DEPTH_WEIGHTING:-1}"
export DR_RLM_DEPTH_WEIGHT_ALPHA="${DR_RLM_DEPTH_WEIGHT_ALPHA:-1.0}"
...
uv run --extra fsdp --python 3.12 -m examples.train.dr_rlm.main_dr_rlm \
  ...
  generator.reward_mode=rao \
  generator.rao_lambda=0.5 \
  ...
  trainer.run_name="dr_rlm_L2_rao_pernode" \
  trainer.log_path="$(pwd)/.neer/artifacts/skyrl-logs" \
  trainer.ckpt_path="$(pwd)/.neer/artifacts/ckpts/dr_rlm_L2" \
