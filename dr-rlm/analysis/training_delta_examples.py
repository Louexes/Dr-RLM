...
generation order; same file `analysis/rl_training_curves.py` consumes) and emits:
...
RUN-AGNOSTIC: this is a PLACEHOLDER built from the v1-clean pilot run. To produce the
...
Usage:
  python analysis/training_delta_examples.py \
      --metrics runs/rl_training/credit_metrics_L3_v1clean.jsonl \
...
%   {metrics_path}
% This is the v1-clean provenance pilot run (relative numbers, frozen corpus).
...
\\begin{{table}}[t]
\\centering
\\caption{{\\textbf{{Training improves Dr-RLM's raw outputs (placeholder, v1-clean pilot run).}}
...
def main():
    ...
    ap.add_argument("--metrics", default="runs/rl_training/credit_metrics_L3_v1clean.jsonl")
